...
# Script: GCP VLESS Deployer with Telegram Bot
# Author: Assistant
# Version: 2.0
# GitHub: https://github.com/khalilvip1/gcp-vless
...
# Check dependencies
check_dependencies() {
    print_info "فحص المتطلبات الأساسية..."
    
    if ! command -v gcloud &> /dev/null; then
        print_error "Google Cloud CLI (gcloud) غير مثبت"
        print_info "الرجاء تثبيته من: https://cloud.google.com/sdk/docs/install"
        exit 1
    fi

    if ! command -v curl &> /dev/null; then
        print_error "curl غير مثبت"
        exit 1
    fi
...
# Create Dockerfile with Xray
create_dockerfile() {
    print_info "جاري إنشاء ملف Dockerfile..."
...
deploy_service() {
    ...
    # Build and deploy
    print_info "جاري بناء الصورة (قد يستغرق 5-10 دقائق)..."
    if ! gcloud builds submit --tag gcr.io/$PROJECT_ID/$SERVICE_NAME --quiet; then
        print_error "فشل في بناء الصورة"
        exit 1
    fi
...
generate_vless_config() {
    ...
    # Create VLESS link
    local vless_link="vless://${UUID}@${domain}:443?path=%2Fvless-${PATH_SUFFIX}&security=tls&alpn=h3%2Ch2%2Chttp%2F1.1&encryption=none&host=${domain}&fp=randomized&type=ws&sni=${domain}#${SERVICE_NAME}"
...
test_service() {
    ...
    if curl -s --retry 3 --retry-delay 2 -f "$service_url" > /dev/null; then
...
EOF
    ...
    print_info "انتظر 30 ثانية لتفعيل الخدمة..."
    sleep 30
    ...
    echo "🔑 UUID: $UUID"
    echo "🛣️ المسار: /vless-$PATH_SUFFIX"
    echo "🔒 الأمان: TLS + WebSocket"
    echo ""
...
• 🛣️ <b>المسار:</b> <code>/vless-$PATH_SUFFIX</code>
